    ...
    <div ref="OperateBox" class="operateBox">
      <el-form :inline="true" :model="formInline" label-position="left" @submit.native.prevent>
        <el-form-item label="策略名称">
          <el-input v-model="formInline.name" clearable size="small" placeholder="请输入名称" @keyup.enter.native="onSearch" />
        </el-form-item>
        <el-form-item>
        ...
        <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="100px" class="demo-ruleForm">
          ...
          <el-col v-show="activeIndex=='2'" :span="21" style="padding:10px">
            <el-form-item label="策略名称" prop="name">
              <el-input v-model="ruleForm.name" placeholder="请输入策略名称" :disabled="disable" />
            </el-form-item>
            ...
            <el-form-item v-show="once" label="通知时段" prop="timeDescription">
              <el-date-picker v-model="ruleForm.timeDescription" type="datetimerange" :disabled="disable" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" />
            </el-form-item>
            <!--        day,week,month,year-->
            ...
            <el-form-item label="备注" prop="remark">
              <el-input v-model="ruleForm.remark" type="textarea" :disabled="disable" placeholder="请输入备注" />
            </el-form-item>
          </el-col>
          <!-- 规则 -->
          <el-col v-show="activeIndex=='3'" :span="21" style="padding: 10px;overflow: auto;height: 530px;">
            <el-form-item label="策略类型" prop="rule_type">
              <el-select v-model="ruleForm.rule_type" placeholder="请选择用途" style="width:30%" :disabled="disable" @change="changeRuleType">
                <el-option label="合并" value="C" />
              </el-select>
            </el-form-item>
            <el-form-item label="优先级" prop="display_order">
              <el-input v-model="ruleForm.displayOrder" clearable placeholder="数字越小优先级高" style="width:30%" :disabled="disable" />
            </el-form-item>
            <div v-if="ruleForm.rule_type === 'C'">
              ...
              <el-form-item label="运算公式" prop="formular">
                <el-input v-model="ruleForm.formular" :disabled="disable" clearable />
              </el-form-item>
              <el-form-item label="条件字段" prop="expression">
                <el-table :show-header="status" :data="ruleForm.expression" style="overflow-x:hidden">
                  <el-table-column label="值" width="150">
                    <template slot-scope="scope">
                      <el-select v-model="scope.row.conditionColumn" clearable size="small" :disabled="disable" placeholder="类型" @change="columnChange(scope.row);setFormular(scope.row.conditionColumn)">
                  ...
                  <el-table-column label="值" width="342">
                    <template slot-scope="scope">
                      <el-date-picker
                      ...
                      <el-input v-else v-model="scope.row.conditionValue[0]" :disabled="disable" placeholder="默认值" :value="scope.row.conditionValue[0]" />
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column label="操作" prop="name" width="35">
                    <template slot-scope="scope">
                      ...
                      <!-- <el-button size="mini" type="text" v-if="ruleForm.expression.length!=1" @click="fieldExpreDelete(scope)" :disabled='disable'>删除</el-button>
                      <el-button size="mini" type="text" @click="addExpreClick()" v-if="scope.$index+1==ruleForm.expression.length || scope.$index==ruleForm.expression.length " :disabled='disable'>新增</el-button> -->
                    </template>
                  </el-table-column>
                </el-table>
              </el-form-item>
              <el-form-item label="合并标识">
                <el-input v-model="ruleForm.effect[0].effectValue" :disabled="disable" clearable placeholder="请输入合并标识" />
              </el-form-item>
            </div>
          </el-col>
        </el-form>
...
<script>
import {
  addHandlerRulesDefine,
  updateHandlerRulesDefine,
  getHandlerRulesDefine,
  deleteHandlerRulesDefine,
  getEventStrategyById,
  getResByCategoryName,
  getPhysicalCategory,
  getProbeSourceType
} from '@/api/cm/eventStrategy'
...
export default {
  ...
  data() {
    return {
      ...
      SeverityArr: [],
      conditions: [
        { column: null, operator: null, value: null, dataType: null }
      ],
      // TIMESTAMP运算符
      ...
      operatorsInt: [
        ...
        { opertorName: '小于', opertorChar: '<' },
        { opertorName: '小于等于', opertorChar: '<=' },
        { opertorName: '大于', opertorChar: '>' },
        { opertorName: '大于等于', opertorChar: '>=' },
        { opertorName: '包含', opertorChar: 'in' },
        { opertorName: '在  和  之间', opertorChar: 'between' },
      ...
      eventLevel: [
        { name: '严重', characte: '=', IsRequired: true, zabbixName: 0 },
        { name: '一般', characte: '=', IsRequired: true, zabbixName: 0 },
        { name: '中等', characte: '=', IsRequired: true, zabbixName: 0 },
        { name: '发生时间2', characte: '=', IsRequired: true, zabbixName: 0 }
      ],
      activeIndex: '2',
      // 获取表格页面高度
      ...
      show: false,
      radios: [
        { label: '日', value: 'day', isChecked: true },
        { label: '周', value: 'week', isChecked: false },
        { label: '月', value: 'month', isChecked: false },
        { label: '年', value: 'year', isChecked: false }
      ],
      sources: [
        { id: 'ZABBIX', name: 'ZABBIX' },
        { id: 'SYSLOG', name: 'SYSLOG' },
        { id: 'SNMPTRAP', name: 'SNMPTRAP' }
      ],
      operators: [
        { opertorName: '等于', opertorChar: '=' },
        { opertorName: '不等于', opertorChar: '!=' },
      ...
      mappingAndJoin: [
        { label: '映射', value: 'mapping' } /*, {label:"关联",value:"join"}*/
      ],
      operatorLogics: [
  ...
  methods: {
    querySeverity() {
      getEventSeverityDefine({
        ...
        this.SeverityArr = this.SeverityArr.filter(
    ...
    changeIntervalType() {
      if (this.ruleForm.interval_type === 'day') {
        ...
        this.ruleForm.execute_at = moment().format('HH:mm:ss')
        this.ruleForm.execute_util = moment()
          .subtract(-1, 'h')
          .format(' HH:mm:ss')
        this.day = true
        this.week = false
    ...
    //         this.ruleForm.formular +
    //         ' and ' +
    //         this.ruleForm.expression[i].conditionColumn
    //     }
    //   }
    // },
    setFormular(row) {
        ...
        if (valid) {
          ...
          if (this.saveOrUpdate === 'save') {
            ...
            if (ruleFormTempObj.rule_type === 'R') {
              ruleFormTempObj.expression = JSON.stringify(
            ...
            if (ruleFormTempObj.exec_type === 'O') {
              ...
              if (ruleFormTempObj.interval_type === 'month') {
                ruleFormTempObj.execute_at =
                  ruleFormTempObj.day_start + ' ' + ruleFormTempObj.execute_at
                ruleFormTempObj.execute_util =
                  ruleFormTempObj.day_end + ' ' + ruleFormTempObj.execute_util
              } else if (ruleFormTempObj.interval_type === 'year') {
                ruleFormTempObj.execute_at =
                  ruleFormTempObj.month_start +
                  '-' +
                  ruleFormTempObj.day_start +
                  ' ' +
                  ruleFormTempObj.execute_at
                ruleFormTempObj.execute_util =
                  ruleFormTempObj.month_end +
                  '-' +
                  ruleFormTempObj.day_end +
                  ' ' +
                  ruleFormTempObj.execute_util
              }
            }
            if (ruleFormTempObj.timeDescription.length > 0) {
              ruleFormTempObj.execute_at = moment(
                ruleFormTempObj.timeDescription[0]
              ).format('YYYY-MM-DD HH:mm:ss')
              ruleFormTempObj.execute_util = moment(
                ruleFormTempObj.timeDescription[1]
              ).format('YYYY-MM-DD HH:mm:ss')
            }
    ...
    handleDelete(row) {
      this.$confirm('是否确认删除该项数据', '警告', {
        ...
        .then(() => {
          const a = row.id
          deleteHandlerRulesDefine({ id: row.id, projectId: 10 }).then(
            (response) => {
    ...
    handleWatch(row) {
      ...
      getEventStrategyById({ id: row.id, projectId: 10 }).then((response) => {
        ...
        if (this.ruleForm.exec_type === 'O') {
          this.once = true
          this.twice = false
          this.$set(this.ruleForm, 'timeDescription', [
            moment(this.ruleForm.execute_at).format('YYYY-MM-DD HH:mm:ss'),
            moment(this.ruleForm.execute_util).format('YYYY-MM-DD HH:mm:ss')
          ])
        } else {
          this.once = false
          this.twice = true
