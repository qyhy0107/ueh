          ...
          <el-col v-show="activeIndex == '3'" :span="20" style="padding: 10px;overflow: auto;height: 530px;">
            ...
            <div v-if="ruleForm.rule_type === 'R'">
              ...
              <el-form-item label="运算公式" prop="formular">
                <el-input v-model.trim="ruleForm.formular" clearable :disabled="true" placeholder="选择条件字段后自动展示" />
              </el-form-item>
              <el-form-item label="条件字段" prop="expression" style="margin-bottom:0!important">
                <template>
                  <el-form ref="formData3" :model="formData3" class="dataForm workForm">
                    <el-form-item label="">
                      <el-select v-model="dictTableValue" style="width: 50%" placeholder="请选择表" clearable size="small" :disabled="disable" @change="getResByCategoryNames(0)">
                        <el-option v-for="item1 in selectTable" :key="item1.dictValue" :label="item1.dictLabel" :value="item1.dictValue" />
                      </el-select>
                    </el-form-item>
                    <el-row v-for="(item, index) in formData3.powerAttrList" :key="index">
                      <el-form-item v-if="dictTableValue" label="">
                        <el-col :span="6" style="margin-right:10px">
                          <el-select v-model="item.conditionColumn" placeholder="请选择条件字段" clearable :disabled="disable" @change="setFormular(item.conditionColumn)">
                            <el-option v-for="item1 in effectExper" :key="item1.columnInDB" :label="item1.columnName" :value="item1.columnInDB" />
                          </el-select>
                        </el-col>
                        <el-col :span="6" style="margin-right:10px">
                          <el-select v-model="item.operator" placeholder="请选择类型" clearable :disabled="disable">
                            <el-option v-for="item1 in mappingAndJoin" :key="item1.value" :label="item1.label" :value="item1.value" />
                          </el-select>
                        </el-col>
                        <el-col :span="6" style="margin-right:10px">
                          <el-select v-model="item.columnName" placeholder="请选择字段" :disabled="disable" clearable>
                            <el-option v-for="item1 in selectColumn" :key="item1.dictValue" :label="item1.dictLabel" :value="item1.dictValue" />
                          </el-select>
                        </el-col>
                        <el-button v-if="formData3.powerAttrList.length > 1" type="text" size="mini" style="color:#3D6EC0" :disabled="disable" @click="removeRow(index)">删除 </el-button>
                        <el-button style="color:#3D6EC0" type="text" size="mini" :disabled="disable" @click="addWork">
                          新增
                        </el-button>
                      </el-form-item>
                      <el-row v-if="item.operator == 'mapping'">
                        <el-table :show-header="status" :data="item.mapping" style="margin-bottom:22px">
                          <el-table-column label="值" class-name="small-padding fixed-width">
                            <template slot-scope="scope">
                              <el-input v-model.trim="scope.row.sourceColumn" :disabled="disable" placeholder="请输入值" :value="scope.row.sourceValue" />
                            </template>
                          </el-table-column>
                          <el-table-column label="映射值" class-name="small-padding fixed-width">
                            <template slot-scope="scope">
                              <el-input v-model.trim="scope.row.sourceValue" :disabled="disable" placeholder="请输入映射值" :value="scope.row.sourceValue" />
                            </template>
                          </el-table-column>
                          <!-- <el-table-column label="映射值" class-name="small-padding fixed-width">
                            <template slot-scope="scope">
                              <el-select v-model="scope.row.columnName" placeholder="请选择字段" :disabled="disable" clearable size="small">
                                <el-option v-for="item1 in selectColumn" :key="item1.dictValue" :label="item1.dictLabel" :value="item1.dictValue" />
                              </el-select>
                            </template>
                          </el-table-column> -->
                          <el-table-column label="操作" prop="name" class-name="small-padding fixed-width">
                            <template slot-scope="scope">
                              <el-button v-if="item.mapping.length != 1" size="mini" type="text" :disabled="disable" @click="fielFengfudExpreDelete(index, scope)">删除 </el-button>
                              <el-button size="mini" type="text" :disabled="disable" @click="addFengfuExpreClick(index)">新增 </el-button>
                            </template>
                          </el-table-column>
                        </el-table>
                      </el-row>
                    ...
                    <el-form-item v-if="dictTableValue" label="">
                      <el-select v-model="resultColumnValue" style="width: 50%" placeholder="请选择返回字段" :disabled="disable" clearable>
                        <el-option v-for="item2 in selectColumn" :key="item2.dictValue" :label="item2.dictLabel" :value="item2.dictValue" />
                      </el-select>
                    </el-form-item>
                  </el-form>
                </template>
              ...
              <el-form-item label="结果字段" prop="effect[0].effectValue">
                <el-col :span="6" style="margin-right:10px">
                  <el-select v-model="ruleForm.effect[0].effectValue" placeholder="请选择结果字段" clearable :disabled="disable">
...
<script>
import { addHandlerRulesDefine, updateHandlerRulesDefine, getHandlerRulesDefine, deleteHandlerRulesDefine, getEventStrategyById, getResByCategoryName, getPhysicalCategory, getProbeSourceType, getSelectDictData } from '@/api/cm/eventStrategy'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        event_source: [{ required: true, message: '请选择数据源', trigger: 'blur' }],
        event_source_type: [{ required: true, message: '请选择数据源', trigger: 'blur' }],
        formular: [{ required: true, message: '请选择条件字段', trigger: 'blur' }],
        rule_type: [{ required: true, message: '请选择策略类型', trigger: 'blur' }],
        'effect[0].effectValue': [{ required: true, message: '请选择结果字段', trigger: 'blur' }]
        // 'effect[0].resultColumnValue': [{ required: true, message: '请选择返回字段', trigger: 'blur' }]
      }
    }
  },
  watch: {
    dictTableValue: function(newVal, oldVal) {
      if (oldVal) {
        ...
        this.formData3.powerAttrList.map((value) => {
          value['tableName'] = newVal
          value['columnName'] = ''
          value['resultColumnValue'] = ''
        })
        this.resultColumnValue = ''
      }
      // this.ruleForm.expression[0].tableName = newVal
      this.getSelectColumnData(this.dictTableValue)
    },
    resultColumnValue: function() {
      this.formData3.powerAttrList.map((value) => {
        value['resultColumnValue'] = this.resultColumnValue
      })
    }
  },
  created() {
    ...
    this.querySeverity()
    this.getSelectTableData('rich_table')
  },
  updated() {
  ...
  methods: {
    ...
    addWork() {
      ...
      this.formData3.powerAttrList.push({
        ...
        tableName: this.dictTableValue || '',
        columnName: this.dictColumnValue || ''
      })
    },
    ...
    // 获取条件字段中的表
    getSelectTableData(dictType) {
      getSelectDictData({ projectId: 10, dictType }).then((response) => {
        this.selectTable = response.datas
      })
    },
    // 获取条件字段中的表
    getSelectColumnData(dictType) {
      getSelectDictData({ projectId: 10, dictType }).then((response) => {
        this.selectColumn = response.datas
      })
    },
    // 删除属性列
    removeRow(index) {
    ...
    setFormular(row) {
      this.ruleForm.formular = ''
      // 根据条件字段拼接运算公式
      for (var i = 0; i < this.formData3.powerAttrList.length; i++) {
        if (this.ruleForm.formular === '') {
          this.ruleForm.formular = this.formData3.powerAttrList[i].conditionColumn
        } else {
          const temp = ' and '
          this.ruleForm.formular = this.ruleForm.formular + temp + this.formData3.powerAttrList[i].conditionColumn
        }
      }
    },
      ...
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.saveOrUpdate === 'save') {
            ...
            this.formData3.powerAttrList[0].tableName = this.dictTableValue
    ...
    handleWatch(row) {
      ...
      getEventStrategyById({ id: row.id, projectId: 10 }).then((response) => {
        ...
        this.dictTableValue = this.formData3.powerAttrList[0]['tableName']
        this.resultColumnValue = this.formData3.powerAttrList[0]['resultColumnValue']
    ...
    handleUpdate(row) {
      ...
      // this.formData3.powerAttrList = JSON.parse(
