  ...
  <div class="app-container">
    <div ref="OperateBox" class="operateBox">
      <el-form :inline="true" :model="formInline" label-position="left" @submit.native.prevent>
        ...
        <el-form-item label="屏蔽类型">
          <el-select v-model="formInline.shield" size="small" clearable placeholder="请选择" style="width:100px" @change="onSearch">
    ...
    <div class="showTableBox" :style="{ height: OperateBoxHeight + 'px' }">
      <el-table :data="dataList" :style="{ height: OperateBoxHeight-60 + 'px' }" empty-text=" " element-loading-text="拼命加载中..." >
        ...
        <el-table-column prop="timeDescription" min-width="12%" label="时间描述" show-overflow-tooltip :formatter="getTimeDescription" />
            ...
            <div v-if="ruleForm.rule_type === 'F'">
              ...
              <el-form-item label="条件字段" prop="expression">
                <el-table :show-header="status" :data="ruleForm.expression">
                      ...
                      <el-form label-position="left" label-width="100px" inline class="demo-table-expand">
                        ...
                        <div>
                          <el-form-item label="操作">
                            ...
                            <el-select v-else-if="props.row.conditionDataType == 'STRING' && props.row.conditionColumn == 'Node'" v-model="props.row.operator" clearable size="small" placeholder="运算符" :disabled="disable">
                            ...
                            <el-select v-else-if="props.row.conditionDataType == 'STRING' && props.row.conditionColumn == 'EventCategory'" v-model="props.row.operator" clearable size="small" placeholder="运算符" :disabled="disable">
                  ...
                  <el-table-column label="值" prop="conditionColumn">
                    <template slot-scope="scope">
                      <el-select
                  ...
                  <!--<el-table-column label="操作">
                    <template slot-scope="scope">
                      <el-select v-model="scope.row.operator" placeholder="请选择类型" clearable size="small" style="width: 150px" :disabled='disable'>
                  ...
                  <el-table-column label="操作">
                    <template slot-scope="scope">
                      ...
                      <el-select v-else-if="scope.row.conditionDataType == 'STRING' && scope.row.conditionColumn == 'EventCategory'" v-model="scope.row.operator" :disabled="disable" clearable size="small" placeholder="运算符">
                        <el-option v-for="operator in operatorsStrEventCategory" :key="operator.opertorChar" :label="operator.opertorName" :value="operator.opertorChar" />
                      </el-select>
                      <el-select v-else-if="scope.row.conditionDataType == 'STRING' && scope.row.conditionColumn == 'Agent'" v-model="scope.row.operator" :disabled="disable" clearable size="small" placeholder="运算符">
                        <el-option v-for="operator in operatorsStrAgent" :key="operator.opertorChar" :label="operator.opertorName" :value="operator.opertorChar" />
                      </el-select>
                      <el-select v-else-if="scope.row.conditionDataType == 'STRING' && scope.row.conditionColumn == 'Manager'" v-model="scope.row.operator" :disabled="disable" clearable size="small" placeholder="运算符">
                        <el-option v-for="operator in operatorsStrManager" :key="operator.opertorChar" :label="operator.opertorName" :value="operator.opertorChar" />
                      </el-select>
                  ...
                  <el-table-column label="逻辑操作" prop="operatorLogic">
                    <template slot-scope="scope">
                      <el-select v-model="scope.row.operatorLogic" placeholder="请继续" clearable size="small" style="width: 80px" :disabled="disable">
                        <el-option v-for="item in operatorLogics" :key="item.opertorLogicChar" :label="item.opertorLogicName" :value="item.opertorLogicChar" />
                      </el-select>
                    </template>
                  </el-table-column>
              ...
              <el-form-item label="屏蔽类型">
                <el-checkbox-group v-model="ruleForm.effect[0].effectValue" :disabled="disable">
                  <!-- <el-checkbox label="NF">不开单</el-checkbox> -->
                  <el-checkbox label="NN">不通知</el-checkbox>
                  <!-- <el-checkbox label="NS">不分享</el-checkbox> -->
                  <el-checkbox label="NE">不展示</el-checkbox>
                </el-checkbox-group>
              </el-form-item>
            </div>
          </el-col>
...
<script>
import { addHandlerRulesDefine, updateHandlerRulesDefine, getHandlerRulesDefine, deleteHandlerRulesDefine, getEventStrategyById, getResByCategoryName, getPhysicalCategory, getProbeSourceType, getManagerSelect, getPhysicalCategory1 } from '@/api/cm/eventStrategy'
...
export default {
  ...
  methods: {
    ...
    getEffectValue(row) {
      // console.log(row)
      const temp = JSON.parse(row.effect)[0]
        .effectValue.join(' 、')
        .replace('NN', '不通知')
        .replace('NF', '不开单')
        .replace('NS', '不分享')
