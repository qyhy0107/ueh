  ...
  <div class="app-container">
    ...
    <div class="showTableBox" :style="{ height: OperateBoxHeight + 'px' }">
      <el-table v-if="dataList.length" :data="dataList" border>
        ...
        <el-table-column prop="name" label="策略名称" show-overflow-tooltip min-width="15%" />
        <!-- <el-table-column prop="rule_type" label="策略类型" :formatter="getRuleType" width="160" align="center" /> -->
        <el-table-column prop="exec_type" label="时间规则" :formatter="getExecType" align="center" min-width="7%" />
        <el-table-column prop="interval_type" label="重复类型" show-overflow-tooltip min-width="7%" />
        <el-table-column prop="timeDescription" label="时间描述" show-overflow-tooltip :formatter="getTimeDescription" min-width="15%" />
        <el-table-column prop="lm_timestamp" label="最后修改时间" show-overflow-tooltip min-width="10%" />
        <el-table-column prop="modifier" label="最后修改人" show-overflow-tooltip min-width="10%" />
        <el-table-column prop="on_completion" label="过期保留" align="center" show-overflow-tooltip min-width="5%">
        ...
        <el-table-column prop="is_enable" label="启用" align="center" show-overflow-tooltip min-width="6%">
          <template slot-scope="scope">
        ...
        <el-table-column prop="remark" label="备注" show-overflow-tooltip min-width="10%" />
          ...
          <el-col v-show="activeIndex == '3'" :span="20" style="padding: 10px;overflow: auto;height: 530px;">
            <el-form-item label="策略类型" prop="rule_type">
              <el-select v-model="ruleForm.rule_type" placeholder="请选择用途" style="width:30%" :disabled="disable" @change="changeRuleType">
                <!--<el-option label="降级" value="D" />-->
                <el-option label="升降级" value="U" />
              </el-select>
            </el-form-item>
            ...
            <div v-if="ruleForm.rule_type === 'U'">
              <el-form-item label="事件源类型" prop="event_source_type">
                <el-select v-model="ruleForm.event_source_type" :disabled="disable" placeholder="请选择事件源" style="width:50%" @change="getProbeSourceTypes()">
              ...
              <el-form-item label="运算公式" prop="formular">
                <el-input v-model.trim="ruleForm.formular" clearable :disabled="true" placeholder="" />
              </el-form-item>
              <el-form-item label="条件字段" prop="expression">
                <el-table :show-header="status" :data="ruleForm.expression">
                      ...
                      <el-form label-position="left" label-width="100px" inline class="demo-table-expand">
                        ...
                        <div>
                          <el-form-item label="操作">
                            ...
                            <el-select v-else-if="props.row.conditionDataType == 'STRING' && props.row.conditionColumn == 'Node'" v-model="props.row.operator" :disabled="disable" clearable size="small" placeholder="运算符">
                            ...
                            <el-select v-else-if="props.row.conditionDataType == 'STRING' && props.row.conditionColumn == 'EventCategory'" v-model="props.row.operator" :disabled="disable" clearable size="small" placeholder="运算符">
                            ...
                            <el-select v-else-if="props.row.conditionDataType == 'STRING' && props.row.conditionColumn == 'Agent'" v-model="props.row.operator" :disabled="disable" clearable size="small" placeholder="运算符">
                            ...
                            <el-select v-else-if="props.row.conditionDataType == 'STRING' && props.row.conditionColumn == 'Manager'" v-model="props.row.operator" :disabled="disable" clearable size="small" placeholder="运算符">
                        ...
                        <div>
                          <el-form-item label="值" class="labelSP">
                            <el-date-picker v-if="props.row.conditionDataType == 'LONGTIMESTAMP'" v-model="props.row.conditionValue[0]" :disabled="disable" value-format="timestamp" size="small" type="datetimerange" range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" clearable class="specialDateClass" />
                  ...
                  <el-table-column label="值">
                    <template slot-scope="scope">
                      <el-select
                        ...
                        "
                      >
                        <el-option v-for="item in effectExper" :key="item.columnName" :label="item.columnName" :value="item.columnInDB" />
                      </el-select>
                    </template>
                  </el-table-column>
                  <!--<el-table-column label="操作">
                    <template slot-scope="scope">
                      <el-select v-model="scope.row.operator" placeholder="请选择类型" clearable size="small" style="width: 150px">
                        <el-option v-for="item in operators" :label="item.opertorName" :value="item.opertorChar" :key="item.opertorName" />
                      </el-select>
                    </template>
                  </el-table-column>
                  <el-table-column label="值">
                    <template slot-scope="scope">
                      <el-input :disabled='disable' placeholder="请输入默认值" v-model="scope.row.conditionValue" :value="scope.row.conditionValue" />
                    </template>
                  </el-table-column>-->
                  <el-table-column label="操作">
                    <template slot-scope="scope">
                      <el-select v-if="scope.row.conditionDataType == 'INT' || scope.row.conditionDataType == 'FLOAT'" v-model="scope.row.operator" :disabled="disable" clearable size="small" placeholder="运算符">
                        <el-option v-for="operator in operatorsInt" :key="operator.opertorChar" :label="operator.opertorName" :value="operator.opertorChar" />
                      </el-select>
                      ...
                      <el-select v-else-if="scope.row.conditionDataType == 'STRING' && scope.row.conditionColumn == 'Node'" v-model="scope.row.operator" :disabled="disable" clearable size="small" placeholder="运算符">
                        <el-option v-for="operator in operatorsStrNode" :key="operator.opertorChar" :label="operator.opertorName" :value="operator.opertorChar" />
                      </el-select>
                  ...
                  <el-table-column label="值">
                    <template slot-scope="scope">
                      <el-date-picker v-if="scope.row.conditionDataType == 'LONGTIMESTAMP'" v-model="scope.row.conditionValue[0]" :disabled="disable" value-format="timestamp" size="small" type="datetimerange" range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" clearable class="specialDateClass" />
                  ...
                  <el-table-column label="逻辑操作">
                    <template slot-scope="scope">
                      <el-select v-model="scope.row.operatorLogic" placeholder="请继续" clearable :disabled="disable" size="small" style="width: 150px">
                        <el-option v-for="item in operatorLogics" :key="item.opertorLogicChar" :label="item.opertorLogicName" :value="item.opertorLogicChar" />
                      </el-select>
                    </template>
                  </el-table-column>
              ...
              <el-form-item label="结果" prop="effect[0].effectValue">
                <el-select v-model="ruleForm.effect[0].effectValue" :disabled="disable" size="small" placeholder="级别内容" clearable style="width:30%">
                  <el-option v-for="SItem in SeverityArr" :key="SItem.mappingValue" :value="SItem.mappingValue" :label="SItem.name" />
                </el-select>
              </el-form-item>
            </div>
            <div v-if="ruleForm.rule_type === 'D'">
              <el-form-item label="事件源类型" prop="event_source_type">
                <el-select v-model="ruleForm.event_source_type" :disabled="disable" placeholder="请选择事件源" style="width:50%" @change="getProbeSourceTypes()">
                  <el-option v-for="item in eventSourceTypeSelect" :key="item.dictLabel" :label="item.dictLabel" :value="item.dictValue" />
                </el-select>
              ...
              <el-form-item label="运算公式" prop="formular">
                <el-input v-model.trim="ruleForm.formular" clearable :disabled="true" />
              </el-form-item>
              <el-form-item label="条件字段" prop="expression">
                <el-table :show-header="status" :data="ruleForm.expression">
                  <el-table-column type="expand">
                  ...
                  <el-table-column label="值">
                    <template slot-scope="scope">
                      <el-date-picker v-if="scope.row.conditionDataType == 'LONGTIMESTAMP'" v-model="scope.row.conditionValue[0]" value-format="timestamp" size="small" type="datetimerange" range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" clearable class="specialDateClass" />
                      <el-input-number v-else-if="(scope.row.conditionDataType == 'INT' || scope.row.conditionDataType == 'FLOAT') && scope.row.operator != 'between' && scope.row.conditionColumn != 'Severity' && scope.row.conditionColumn != 'OldSeverity'" v-model="scope.row.conditionValue[0]" size="small" placeholder="请输入内容" clearable style="width:100%" controls-position="right" :min="1" />
                      <el-select v-else-if="(scope.row.conditionDataType == 'INT' || scope.row.conditionDataType == 'FLOAT') && scope.row.operator != 'between' && (scope.row.conditionColumn == 'Severity' || scope.row.conditionColumn == 'OldSeverity') && scope.row.operator !== 'in'" v-model="scope.row.conditionValue[0]" :disabled="disable" size="small" :multiple="false" placeholder="请选择级别内容" clearable>
                        <el-option v-for="SItem in SeverityArr" :key="SItem.mappingValue" :value="SItem.mappingValue" :label="SItem.name" />
                      </el-select>
                      <el-select v-else-if="(scope.row.conditionDataType == 'INT' || scope.row.conditionDataType == 'FLOAT') && scope.row.operator != 'between' && (scope.row.conditionColumn == 'Severity' || scope.row.conditionColumn == 'OldSeverity') && scope.row.operator == 'in'" v-model="scope.row.conditionValue" :disabled="disable" size="small" :multiple="true" placeholder="请选择级别内容" clearable>
                      ...
                      <el-row v-else-if="(scope.row.conditionDataType == 'INT' || scope.row.conditionDataType == 'FLOAT') && scope.row.operator == 'between' && (scope.row.conditionColumn != 'Severity' || scope.row.conditionColumn != 'OldSeverity')" :gutter="4">
                        <el-col :span="11">
                          <el-input-number v-model="scope.row.conditionValue[0]" size="small" placeholder="最小值" clearable style="width:100%" controls-position="right" :min="1" />
                        </el-col>
                        <el-col :span="2" style="text-align: center; line-height: 2rem">
                          <span>-</span>
                        </el-col>
                        <el-col :span="11">
                          <el-input-number v-model="scope.row.conditionValue[1]" size="small" placeholder="最大值" clearable style="width:100%" controls-position="right" :min="1" />
                        </el-col>
                      </el-row>
                      <el-input v-else v-model.trim="scope.row.conditionValue[0]" :disabled="disable" placeholder="默认值" :value="scope.row.conditionValue[0]" />
                    </template>
                  ...
                  <!--                  <el-table-column label="操作">
                    <template slot-scope="scope">
                      <el-select v-model="scope.row.operator" placeholder="请选择类型" clearable size="small" style="width: 150px">
                        <el-option v-for="item in operators" :label="item.opertorName" :value="item.opertorChar" :key="item.opertorChar" />
                      </el-select>
                    </template>
                  </el-table-column>
                  <el-table-column label="值">
                  ...
                  <el-table-column label="逻辑操作">
                    <template slot-scope="scope">
                      <el-select v-model="scope.row.operatorLogic" placeholder="请继续" clearable size="small" style="width: 150px" :disabled="disable">
                        <el-option v-for="item in operatorLogics" :key="item.opertorLogicChar" :label="item.opertorLogicName" :value="item.opertorLogicChar" />
                      </el-select>
                    </template>
                  </el-table-column>
...
<script>
import { addHandlerRulesDefine, updateHandlerRulesDefine, getHandlerRulesDefine, deleteHandlerRulesDefine, getEventStrategyById, getResByCategoryName, getPhysicalCategory, getProbeSourceType } from '@/api/cm/eventStrategy'
...
import { getEventSourceTypeSelect } from '@/api/cm/eventAccess'
export default {
  ...
  data() {
    return {
      ...
      effectExper: [],
      effectTypes: [{ key: 'counter', value: '累计' }, { key: 'newest', value: '最新' }],
      resource_types: '',
      resource_names: '',
      eventSources: '',
      event_source: [],
      ruleForm: {
        ...
        remark: '',
        displayOrder: 0,
        params: { cycleTime: 60, isLongCycleCompress: 'Y' }
      },
      saveOrUpdate: 'save',
      once: true,
      twice: false,
  ...
  methods: {
    ...
    getEventColumnDefine() {
      getEventColumnDefine({
        ...
        projectId: 10,
        engine: 'Freemarker',
        columnTypes: new Array('U', 'C') // 内置可修改   自定义字段
      }).then((response) => {
