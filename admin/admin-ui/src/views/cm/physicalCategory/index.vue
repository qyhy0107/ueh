  ...
  <div class="app-container">
    <div class="operateBox">
      <el-form ref="queryForm" :model="queryParams" :inline="true" label-position="left">
        <el-form-item label="设备类别" prop="name">
          <el-input v-model.trim="queryParams.name" placeholder="请输入指标设备类别或显示名称" clearable size="small"
        ...
        <el-form-item>
          ...
          <el-button class="grayButton" v-hasPermi="['system:config:add']" icon="el-icon-plus" size="mini"
    ...
    <div class="showTableBox" :style="{height:OperateBoxHeight+'px'}">
      <el-table  v-loading="loading" :data="configList" row-key="id"
        :tree-props="{children: 'children', hasChildren: 'hasChildren'}" :style="{height:OperateBoxHeight-60+'px'}"  empty-text=" " element-loading-text="拼命加载中...">
        ...
        <el-table-column :show-overflow-tooltip="true" prop="parentName" header-align="left" align="left"
          label="上级类别" />
        <el-table-column :show-overflow-tooltip="true" prop="shortName" header-align="left" align="left" label="短名称" />
        <el-table-column :show-overflow-tooltip="true" prop="displayOrder" header-align="left" align="left"
          label="显示顺序" />
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button v-hasPermi="['system:group:edit']" size="mini" type="text" icon="el-icon-edit"
              @click="handleUpdate(scope.row)">修改</el-button>
            <el-button v-hasPermi="['system:group:remove']" size="mini" type="text" icon="el-icon-delete"
              @click="handleDelete(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <pagination v-show="total>0&&configList.length" :page-sizes="[10, 20, 50, 200]" :total="total" :page.sync="queryParams.start"
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="90px">
        <el-form-item label="上级类别" prop="parentId">
          <el-select v-model="form.parentId" placeholder="请选择上级类别" clearable filterable>
            <el-option v-for="item in physicalCategoryOptions" :key="item.id" :label="item.labelText" :value="item.id"
              :style="{display:item.parentId===0?'block':'none'}" />
          </el-select>
        ...
        <el-form-item label="显示顺序" prop="displayOrder">
          <el-input-number v-model="form.displayOrder" type="displayOrder" :min="1000" :step="10" placeholder="显示顺序"
