  ...
  <div>
    ...
    <div v-show="!performanceLoading" v-loading="performanceLoading" class="cpuBox">
        ...
        <el-row>
          <el-col>
            <el-form-item label="性能指标">
              <el-select v-model="selectedkeyOps" :popper-append-to-body="false" placeholder="请选择指标" filterable style="width: 260px;margin-right: 10px" size="small">
                <el-option v-for="keyOpss in keyOps" :key="keyOpss.kpi_name" :label="keyOpss.label_text" :value="keyOpss.kpi_name" />
              </el-select>
              <!-- <el-tooltip v-if="tags.length" class="item" effect="dark" content="点击选择指标tag" placement="top"> -->
              <el-select v-if="tags.length" v-model="selectedTag" collapse-tags multiple :popper-append-to-body="false" clearable placeholder="请选择指标tag" filterable style="width: 280px" size="small">
            ...
            <el-col class="timebox">
              ...
              <div style="float:right;margin-right: 20px">
                ...
                <el-tooltip class="item" effect="dark" content="点击导出查询数据" placement="top">
                  <svg-icon icon-class="link" class="el-input__icon" style="color:#c0d0d0;" @click="exportSelected" />
                </el-tooltip>
              </div>
            </el-col>
          </el-row>
      ...
      <div v-if="selectedEct == 'table'" v-loading="performanceLoading1" style="padding-bottom: 10px" element-loading-text="拼命加载中..." empty-text=" ">
        ...
        <pagination v-show="totalCount > 0" :popper-append-to-body="false" :limit.sync="queryParams.limit" :page.sync="queryParams.start" :total="totalCount" :page-sizes="[10, 20, 50, 200]" @pagination="getlist({ host: host, startTime, endTime })" />
      </div>
      <!--echarts图表-->
...
import { getIsKeyDatas, getKeyDatas, getKpiGroup, getKeyDataList, getKeyDataTags, exportPerformanceData } from '@/api/operation/server'
export default {
  components: {
    Deficiency,
    TimeLineChart
  ...
  methods: {
    ...
    selectCardDate(flag) {
      ...
      if (flag) {
        ...
        // var { startTime, endTime } = this.getUnixTimeStamp('1h')
        this.timecheck = '1h'
        this.dateSelected = ''
        this.startTime = startTime
        this.endTime = endTime
      ...
      this.exportParams = {
        ...
        mappingValue: JSON.parse(this.keyOpsObj[this.selectedkeyOps].mappingValue),
      ...
      // console.log(this.selectedkeyOps, this.keyOpsObj)
      // 3.1封装图表数据
      if (this.selectedEct !== 'table') {
        getKeyDatas({
          ...
          getKeyDataTags({
            ...
            if (tags.success && tags.listData.length > 1) {
              this.tags = tags.listData
            } else {
              this.tags = []
            }
          })
    ...
    getSecondTabDatas() {
      ...
      getKpiGroup({
        ...
        getIsKeyDatas({
          ...
          this.keyOps = response.datas
          this.selectedkeyOps = response.datas[0]['kpi_name'] || ''
          this.selectedkeyOpsLable = response.datas[0]['label_text'] || ''
          // 3获取对应指标下的性能数据
          // this.selectCardDate('1h')
          this.selectCardDate(this.timecheck)
        })
    ...
    // 获取表格
    getlist(params) {
      const { host, startTime, endTime } = params
      this.performanceLoading1 = true
      this.perfDatas = []
      // console.log(this.selectedkeyOps)
      getKeyDataList({
        kpi_name: this.selectedkeyOps,
        display_operator: this.keyOpsObj[this.selectedkeyOps].display_operator,
        display_offset_value: this.keyOpsObj[this.selectedkeyOps].display_offset_value,
