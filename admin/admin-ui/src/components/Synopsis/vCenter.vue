    ...
    <div v-if="ww" class="specialBox specialBox1 overviewBox Synopsis">
      ...
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          <span
            >设备状况<span style="font-size:10px!important">（包含vCenter,集群,宿主机,VM,Datastore）<span /> </span
          ></span>
        </div>
        <div class="topLeft">
        ...
        <el-tabs v-model="activeName" v-loading="top5loading" type="card">
          <el-tab-pane v-for="item in top_list" :key="item.topN_title" :label="item.topN_title" :name="item.topN_title">
            <div v-if="activeName == item.topN_title && linedata.options.length > 0" style="height: 100%">
              <common-chart :chart-data="linedata" :chart-height="ww > 1400 ? '100%' : '140px'" :is-router="true" :class_id="265" :class-id-list="kpiDatasetName.classIdList" />
            </div>
          ...
          <el-collapse-item title="设备列表" name="1">
              ...
              <el-form :inline="true" label-position="left" style="height:30px!important">
                ...
                <transition name="el-zoom-in-center">
                  <el-form-item v-if="show1" label="设备标识/名称:">
                    <el-input v-model="resourceName" size="mini" placeholder="请输入查询内容" clearable @keyup.enter.native="findByresourceName" />
                  </el-form-item>
                </transition>
            ...
            <div class="tableBox-right">
              <el-table v-loading="loading1" :data="dataList" empty-text=" " :height="ww > 1400 ? 410 : 265" element-loading-text="拼命加载中..." border @row-dblclick="turnPage" @sort-change="getOrder">
                ...
                <el-table-column align="left" label="" prop="concern" :show-overflow-tooltip="true" min-width="2%">
                  <template slot-scope="scope">
                    ...
                    <!-- <i v-if="scope.row.concern == '0'" class="el-icon-star-off" @click="turnConcern(scope.row)" />
                    <i v-if="scope.row.concern == '1'" class="el-icon-star-on" style="color: rgb(24, 144, 255)" @click="turnConcern(scope.row)" /> -->
                ...
                <el-table-column align="left" label="设备类别" prop="clabel_text" :show-overflow-tooltip="true" sortable="custom" min-width="10%" />
                <el-table-column align="left" label="运行状态" prop="operational_status" :show-overflow-tooltip="true" sortable="custom" min-width="10%">
                  <template slot-scope="scope">
                ...
                <el-table-column align="left" label="设备标识" prop="host" :show-overflow-tooltip="true" sortable="custom" min-width="10%">
                  <template slot-scope="scope">
                ...
                <el-table-column align="left" label="显示名称" prop="label_text" :show-overflow-tooltip="true" sortable="custom" min-width="15%" />
                <!-- 传入不同的指标插槽 -->
                <slot name="tableColumn" />
                <!-- <el-table-column align="left" label="设备名称" prop="label_text" :show-overflow-tooltip="true" sortable="custom" :min-width="240" />
                ...
                <el-table-column align="left" label="运行时长" prop="time" :show-overflow-tooltip="true" sortable="custom" :min-width="200">
          ...
          <el-collapse-item title="告警列表" name="2">
            <div class="collapseBox">
              <el-form :inline="true" label-position="left" style="height:30px!important" @submit.native.prevent>
                ...
                <!-- <transition name="el-zoom-in-center">
                  <el-form-item v-show="show2" label="事件类型:">
                    <el-select v-model="isComponent" size="mini" style="width:100px" clearable @change="getAlarmList">
...
<script>
import CommonChart from '@/views/dashboard/commonChartVC'
...
import { getNumByAlarmLevel, getNumByAlarmState, getTop5DataGroupByHost, getAlarmList, updatePropertyconcern, getAlarmCategory, getResourceSonClassId } from '@/api/operation/serverOverview'
import { getVMResource, getVcenterOverview, getVCTotalAlarmAsset, getVCKeys } from '@/api/operation/vcenter'
export default {
  name: 'Synopsis',
  // props: {
  //   className: {
  ...
  data() {
    return {
      ...
      // 指标数据集存储
      // kpiDatasetName: {
      //   // 6: [getVMResource, getVMResourceByRole]
      //   cpuList: [],
      //   memList: [],
      //   outTrafficList: [],
      //   inTrafficList: [],
  ...
  methods: {
    ...
    getVcenterOverviews() {
      ...
      if (isAdmin) {
        getVcenterOverview({
          ...
          this.getVCTotalAlarmAssets() // 设备告警数
        })
      } else {
        getVcenterOverview({
          class_id: this.class_id,
    ...
    /** 告警设备数 */
    getVCTotalAlarmAssets() {
      getVCTotalAlarmAsset({
        resourceKeys: this.resourceKeys,
        projectId: 10,
        nowDate: this.nowDate1,
        engine: 'Freemarker'
    ...
    getResource() {
      ...
      getVMResource({
        ...
        clusterList: this.kpiDatasetName.clusterList, // 集群个数
