  ...
  <div class="app-container">
    <div v-if="ww" class="specialBox specialBox1 overviewBox Synopsis">
      ...
      <el-card class="box-card">
        ...
        <div class="topLeft">
          <div class="topLeft-item" style="cursor: pointer;" @click="turnPage2('自定义监控', { class_id: 'WBP' })">
        ...
        <el-tabs v-model="activeName" v-loading="top5loading" type="card">
          <el-tab-pane v-for="item in top_list" :key="item.topN_title" :label="item.topN_title" :name="item.topN_title">
            <div v-if="activeName == item.topN_title && linedata.options.length > 0" style="height: 100%">
              <common-chart class-name="x-axis-tip" :chart-data="linedata" :chart-height="ww > 1400 ? '100%' : '140px'" :is-router="true" :class_id="class_id" :class-id-list="kpiDatasetName.classIdList" />
            </div>
      ...
      <el-card id="scrolldIV" class="box-card">
        <!-- <button @click="addScroll">1</button> -->
        <el-collapse v-model="activeCollapse">
          <el-collapse-item title="WEB列表" name="1">
            <div class="collapseBox">
              <el-form :inline="true" label-position="left" style="height:30px!important" @submit.native.prevent>
                ...
                <transition name="el-zoom-in-center">
                  <el-form-item v-if="show1" label="名称/发起设备/被检测的WEB:">
                ...
                <transition v-if="show1" name="el-zoom-in-center">
                  <el-form-item label="运行状态:">
                    <el-select v-model="operational_status" collapse-tags size="mini" clearable @change="findByresourceName">
                      <el-option label="良好" value="Y" />
            ...
            <div class="tableBox-right">
              <el-table v-loading="loading1" :data="dataList" empty-text=" " :height="ww > 1400 ? 410 : 265" element-loading-text="拼命加载中..." border @row-dblclick="turnPage" @sort-change="getOrder">
                ...
                <el-table-column align="left" label="名称" min-width="12%" prop="monitor_name" :show-overflow-tooltip="true" sortable="custom">
                ...
                <el-table-column align="left" label="运行状态" min-width="8%" prop="WEBSTATUS" :show-overflow-tooltip="true" sortable="custom">
                  <template slot-scope="scope">
                    <span v-if="scope.row.WEBSTATUS == 'Y'" class="spanbg1">良 好</span>
                    <span v-else class="spanbg3">异 常</span>
                  </template>
                </el-table-column>
                <el-table-column align="left" label="被检测的WEB" min-width="15%" prop="monitored_host" :show-overflow-tooltip="true" sortable="custom" />
                <!-- <template slot-scope="scope">
                    <el-tooltip class="item" effect="dark" content="点击跳转" placement="top">
                      <span class="tagLink" @click="turnPage(scope.row)">
                        <i class="el-icon-paperclip" />
                ...
                <el-table-column align="left" label="发起设备" min-width="10%" prop="monitored_proxy" :show-overflow-tooltip="true" sortable="custom" />
                <el-table-column align="left" label="URL所在服务器" min-width="10%" prop="monitored_serverIP" :show-overflow-tooltip="true" sortable="custom">
                  <template slot-scope="scope">
                    {{ scope.row.monitored_serverIP || '-' }}
                  </template>
                </el-table-column>
                <!-- <el-table-column align="left" label="关联应用" min-width="10%" prop="monitored_product" :show-overflow-tooltip="true" sortable="custom" /> -->
                <el-table-column align="left" label="响应状态" min-width="10%" prop="WBP_INFO_RESPONSE_STATUS" :show-overflow-tooltip="true" sortable="custom" />
                <el-table-column align="left" label="响应时长(ms)" min-width="10%" prop="WBP_TM_TTL_TIME" :show-overflow-tooltip="true" sortable="custom">
                  <template slot-scope="scope">
                    {{ scope.row.WBP_TM_TTL_TIME | toDecimal }}
                  </template>
                </el-table-column>
                <el-table-column align="left" label="上行速率（Mbps)" min-width="10%" prop="WBP_SPD_UPLOAD" :show-overflow-tooltip="true" sortable="custom">
                  <template slot-scope="scope">
                    {{ scope.row.WBP_SPD_UPLOAD | getMMUnit1 }}
                  </template>
                </el-table-column>
                <el-table-column align="left" label="下行速率（Mbps)" min-width="10%" prop="WBP_SPD_DOWNLOAD" :show-overflow-tooltip="true" sortable="custom">
                  <template slot-scope="scope">
                    {{ scope.row.WBP_SPD_DOWNLOAD | getMMUnit1 }}
                  </template>
                </el-table-column>
                <el-table-column align="left" label="最后一次检测时间" min-width="15%" prop="WBP_INFO_LAST_CHECK_TIMESTAMP" :show-overflow-tooltip="true" sortable="custom">
                  <template slot-scope="scope">
                    {{ scope.row.WBP_INFO_LAST_CHECK_TIMESTAMP | getStartDate }}
                  </template>
                </el-table-column>
                <!-- 传入不同的指标插槽 -->
      ...
      linedata: {
        ...
        tooltip: {
          ...
          formatter: function(value) {
            // console.log(value)
            const a = value[0].axisValue.split('_')[0]
            if (a.length > 31 && a.length < 61) {
              return a.substr(0, 31) + '<br>' + a.substr(31, 60) + '<br>' + '<div style="color: #10aaf4;float:right;font-weight: 700;">' + value[0].data + '' + value[0].seriesName + '</div>'
              // return value[0].axisValue.split('_')[0] + '<br>' + value[0].data + '' + value[0].seriesName
            } else if (a.length >= 61) {
              return a.substr(0, 31) + '<br>' + a.substr(31, 60) + '<br>' + a.substr(60) + '<br>' + '<div  style="color:#10aaf4;float:right;font-weight: 700;">' + value[0].data + '' + value[0].seriesName + '</div>'
            } else {
              return value[0].axisValue.split('_')[0] + '<br>' + '<div  style="color: #10aaf4;float:right;font-weight: 700;">' + value[0].data + '' + value[0].seriesName + '</div>'
            }

            // const a = value.split('_')[0]
            // if (a.length > 40) {
            //   return a.substr(0, 40) + '...'
            // } else {
            //   return a
            // }
          }
  ...
  methods: {
    ...
    turnPage(row) {
      ...
      const query = {
        name: {
          id: row.id,
          clabel_text: 'WEB检测',
          monitored_serverIP: row.monitored_serverIP,
    ...
    getResource() {
      ...
      getWEBResource({
        ...
        class_id: this.class_id,
        userId,
        WEBSTATUS: this.operational_status,
