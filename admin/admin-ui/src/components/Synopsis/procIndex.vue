  ...
  <div class="app-container">
    <div v-if="ww" class="specialBox specialBox1 overviewBox Synopsis">
      ...
      <el-card class="box-card">
        ...
        <div class="topLeft">
          <div class="topLeft-item" style="cursor: pointer;" @click="turnPage2('自定义监控', { class_id: 'PROC' })">
      ...
      <el-card id="scrolldIV" class="box-card">
        <el-collapse v-model="activeCollapse">
          <el-collapse-item title="进程列表" name="1">
            <div class="collapseBox">
              <el-form :inline="true" label-position="left" style="height:30px!important" @submit.native.prevent>
                ...
                <transition name="el-zoom-in-center">
                  <el-form-item v-if="show1" label="名称/被检测的设备:">
            ...
            <div class="tableBox-right">
              <el-table v-loading="loading1" :data="dataList" empty-text=" " :height="ww > 1400 ? 410 : 265" element-loading-text="拼命加载中..." border @row-dblclick="turnPage" @sort-change="getOrder">
                ...
                <el-table-column align="left" label="名称" prop="monitor_name" :show-overflow-tooltip="true" sortable="custom" min-width="20%">
                ...
                <el-table-column align="left" label="运行状态" prop="PROCSTATUS" :show-overflow-tooltip="true" min-width="10%" sortable="custom">
                  <template slot-scope="scope">
                    <span v-if="scope.row.PROCSTATUS == 1" class="spanbg1">良 好</span>
                ...
                <el-table-column align="left" label="进程标识" prop="proc_flag" :show-overflow-tooltip="true" sortable="custom" min-width="10%" />
                <el-table-column align="left" label="进程状态" prop="PROC_INFO_OPERATIONAL_STATUS" :show-overflow-tooltip="true" sortable="custom" min-width="10%">
                  <template slot-scope="scope">
                    <span v-if="scope.row.PROC_INFO_OPERATIONAL_STATUS === 1">正常</span>
                    <span v-else-if="scope.row.PROC_INFO_OPERATIONAL_STATUS === 0">异常</span>
                    <span v-else>-</span>
                  </template>
                </el-table-column>
                <el-table-column align="left" label="CPU使用率(%)" prop="PROC_LD_MEM_PERCENTAGE" :show-overflow-tooltip="true" sortable="custom" min-width="10%">
                  <template slot-scope="scope">
                    {{ scope.row.PROC_LD_MEM_PERCENTAGE | toDecimal }}
                  </template>
                </el-table-column>
                <el-table-column align="left" label="内存使用率(%)" prop="PROC_LD_CPU_PERCENTAGE" :show-overflow-tooltip="true" sortable="custom" min-width="10%">
                  <template slot-scope="scope">
                    {{ scope.row.PROC_LD_CPU_PERCENTAGE | toDecimal }}
                  </template>
                </el-table-column>
                <!-- 传入不同的指标插槽 -->
...
import { getCustomRouter, getWEBResource, getNumByAlarmLevel, getNumByAlarmState, getCustomTop5DataGroupByProc, getCustomOverview, getResourceCount, getTotalAlarmAsset, getCustomAlarmList, getCustomKeys, getResourceSonClassId } from '@/api/operation/serverOverview'
export default {
  name: 'Synopsis',
  ...
  methods: {
    ...
    getNumByAlarmLevel() {
      // 根据告警级别分组获取资源告警数量
      const tempArr = this.resourceKeys.map((item) => {
        return '进程[' + item + ']'
      })
      getNumByAlarmLevel({
        resourceKeys1: tempArr,
        projectId: 10,
        type: '进程检测',
        // nowDate: moment()
        //   .startOf('day')
    ...
    turnPage1(row) {
      ...
      const proc_flag = row.Summary.split(']')[0].replace('进程[', '') || null
      getCustomRouter({
        // monitored_proxy: ip,
    ...
    getResource() {
      ...
      getWEBResource({
        ...
        resourceName: this.resourceName,
        PROCSTATUS: this.operational_status,
        nowDate: this.nowDate1,
        class_id_son: this.class_id_son,
        class_id: this.class_id,
        userId,
