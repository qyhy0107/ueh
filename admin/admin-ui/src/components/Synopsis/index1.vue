<template>
  <div class="app-container">
    <div v-if="ww" class="specialBox specialBox1 overviewBox Synopsis overviewBoxonTopN">
      <!-- 设备状况 -->
      <el-card class="box-card">
        ...
        <div class="topLeft">
          <div class="topLeft-item" style="cursor: pointer;" @click="turnPage2('设备管理', { class_id: class_id })">
            <span>{{ totalAssets }}</span>
            <span>设备总数</span>
          </div>
          <div class="topLeft-item" style="cursor: pointer;" @click="getAbnormalDevices">
        ...
        <el-collapse v-model="activeCollapse">
          <el-collapse-item title="设备列表" name="1">
            <div class="collapseBox">
              <el-form :inline="true" label-position="left" style="height:30px!important">
                <el-form-item>
                ...
                <transition name="el-zoom-in-center">
                  <el-form-item v-if="show1" label="设备标识/名称:">
                ...
                <transition v-if="show1" name="el-zoom-in-center">
                  <el-form-item label="运行状态:">
                    <el-select v-model="operational_status" collapse-tags multiple size="mini" clearable @change="findByresourceName">
                      <el-option label="设备异常-灰" value="0" />
                      <el-option label="设备良好" value="1" />
                      <el-option label="设备异常-橙" value="2" />
                      <el-option label="设备异常-红" value="3" />
                    </el-select>
                  </el-form-item>
                </transition>
                <transition v-if="show1" name="el-zoom-in-center">
                  <el-form-item label="设备类别:">
                    <el-select v-model="class_id_son" size="mini" clearable @change="findByresourceName">
                      <!-- <el-option/> -->
                      <el-option v-for="item in categoryList" :key="item.id" :label="item.label_text" :value="item.id" />
                    </el-select>
                  </el-form-item>
                </transition>
                <transition v-if="show1" name="el-zoom-in-center">
            ...
            <div class="tableBox-right">
              <el-table v-loading="loading1" :data="dataList" empty-text=" " :height="ww > 1400 ? 410 : 265" element-loading-text="拼命加载中..." border @row-dblclick="turnPage" @sort-change="getOrder">
                ...
                <el-table-column align="left" label="" prop="concern" :show-overflow-tooltip="true" :width="30">
                  <template slot-scope="scope">
                    <el-tooltip v-if="scope.row.concern === 0" class="item" effect="dark" content="点击关注" placement="top">
                      <i class="el-icon-star-off" @click="turnConcern(scope.row)" />
                    </el-tooltip>
                    <el-tooltip v-if="scope.row.concern === 1" class="item" effect="dark" content="点击取消关注" placement="top">
                      <i class="el-icon-star-on" style="color: rgb(24, 144, 255)" @click="turnConcern(scope.row)" />
                    </el-tooltip>
                    <!-- <i v-if="scope.row.concern == '1'" class="el-icon-star-on"  @click="turnConcern(scope.row)" /> -->
                    <!-- <el-button v-if="scope.row.concern == '0'" type="text" size="mini" icon="el-icon-plus" @click="turnConcern(scope.row)" /> -->
                    <!-- <el-button v-if="scope.row.concern == '1'" type="text" size="mini" icon="el-icon-minus" class="dangerButton" @click="turnConcern(scope.row)" /> -->
                  </template>
                </el-table-column>
                <el-table-column align="left" label="设备类别" prop="clabel_text" :show-overflow-tooltip="true" :width="130" sortable="custom" />
                <el-table-column align="left" label="运行状态" prop="operational_status" :show-overflow-tooltip="true" :width="120" sortable="custom">
                  <template slot-scope="scope">
                    <span v-if="!scope.row.operational_status" class="spanbg0">暂 无 状 态</span>
                    <span v-else-if="scope.row.operational_status === '0'" class="spanbg0">设 备 异 常</span>
                    <span v-else-if="scope.row.operational_status === '1'" class="spanbg1">设 备 良 好</span>
                    <span v-else-if="scope.row.operational_status === '2'" class="spanbg2">设 备 异 常</span>
                    <span v-else-if="scope.row.operational_status === '3'" class="spanbg3">设 备 异 常</span>
                  </template>
                </el-table-column>
                <el-table-column align="left" label="设备标识" prop="host" :show-overflow-tooltip="true" :width="120" sortable="custom">
                  <template slot-scope="scope">
                ...
                <el-table-column align="left" label="显示名称" prop="label_text" :show-overflow-tooltip="true" sortable="custom" :min-width="230" :max-with="280" />
                <!-- 传入不同的指标插槽 -->
                <slot name="tableColumn" />
                <!-- <el-table-column align="left" label="显示名称" prop="label_text" :show-overflow-tooltip="true" sortable="custom" :min-width="240" />
                <el-table-column align="left" label="CPU（%）" prop="cpu" :show-overflow-tooltip="true" sortable="custom" :min-width="120">
                  <template slot-scope="scope">{{ scope.row.cpu | isZero }}</template>
                </el-table-column>
                <el-table-column align="left" label="内存（%）" prop="mem" :show-overflow-tooltip="true" sortable="custom" :min-width="120">
                  <template slot-scope="scope">{{ scope.row.mem | isZero }}</template>
                </el-table-column>
                <el-table-column align="left" label="每秒发送字节数（Kbps）" prop="outTraffic" :show-overflow-tooltip="true" :min-width="200" sortable="custom">
                  <template slot-scope="scope">{{ scope.row.outTraffic | getKbpsUnit }}</template>
                </el-table-column>
                <el-table-column align="left" label="每秒接收字节数（Kbps）" prop="inTraffic" :show-overflow-tooltip="true" :min-width="200" sortable="custom">
                  <template slot-scope="scope">{{ scope.row.inTraffic | getKbpsUnit }}</template>
                </el-table-column>
                <el-table-column align="left" label="连续运行时间" prop="time" :show-overflow-tooltip="true" sortable="custom" :min-width="200">
                  <template slot-scope="scope">{{ scope.row.time | getTimeTableLong }}</template>
                </el-table-column> -->
              </el-table>
              <!-- <Deficiency v-else height="25%" width="auto" style="height:300px"></Deficiency> -->
          ...
          <el-collapse-item title="告警列表" name="2">
            <div class="collapseBox">
              <el-form :inline="true" label-position="left" style="height:30px!important" @submit.native.prevent>
                ...
                <transition name="el-zoom-in-center">
                  <el-form-item v-show="show2" label="级别:">
                    <el-select v-model="level" collapse-tags multiple size="mini" clearable style="width:130px" @change="getAlarmList">
              ...
              <!-- <span type="text" size="mini" style="background:none;color:#5588ed;cursor: pointer;float:right;" @click="turnPage2('历史告警', { class_id: class_id })">更多>></span> -->
            </div>
            <div class="tableBox">
              <div class="tableBox-right">
                <el-table :key="Math.random()" v-loading="loading" :height="ww > 1400 ? 380 : 250" border :data="dataListAlarm" empty-text=" " element-loading-text="拼命加载中..." @row-dblclick="turnPage1" @selection-change="handleSelectionChange">
        ...
        <el-form ref="formData" :model="formData" label-width="120px" class="demo-ruleForm">
          ...
          <el-row>
            ...
            <el-col :span="12">
              <!-- <el-form-item label="持续时长">
                <el-input v-if="formData.row.EventType === 'P'" value="-" />
                <el-input v-else-if="formData.row.EventType === 'S' && formData.row.time2 === formData.row.time1" :value="(momentDate - formData.row.time1) | getdiffLongTime1" />
                <el-input v-else-if="formData.row.EventType === 'S'" :value="(formData.row.time2 - formData.row.time1) | getdiffLongTime1" />
              </el-form-item> -->
              <el-form-item label="持续时长">
                <!-- <el-input v-if="formData.row.EventType === 'P'" value="-" /> -->
                <el-input v-if="formData.row.RecoveredStatus === '未恢复'" :value="(momentDate - formData.row.time1) | getdiffLongTime1" />
...
import { getNetworkResource, getNumByAlarmLevel, getNumByAlarmState, getTop5DataGroupByHost, getServerOverview, getResourceCount, getTotalAlarmAssetNum, getTotalAlarmAsset, getAlarmList, updatePropertyconcern, getKeys, getAlarmCategory, getResourceSonClassId } from '@/api/operation/serverOverview'
export default {
  name: 'Synopsis',
  // props: {
  //   className: {
  ...
  data() {
    return {
      ...
      // 指标数据集存储
      // kpiDatasetName: {
      //   // 6: [getNetworkResource, getNetworkResourceByRole]
      //   cpuList: [],
      //   memList: [],
      //   outTrafficList: [],
      //   inTrafficList: [],
      ...
      linedata: {
        ...
        series: [
          {
            type: 'bar',
            // name: this.top_list[0].xAxisName,
            itemStyle: {
              normal: {
                label: {
                  show: true,
                  position: 'right',
                  // aqq: '11',
                  // seriesName: this.linedata.xAxis.name,
                  // formatter: '{c}{a|{a}}',
                  formatter: '{c} {a}',
  ...
  watch: {
    $route: {
      handler(val, oldval) {
        const temp = {
          ...
          operational_status: this.operational_status ? (this.operational_status.length ? this.operational_status : '') : '',
    ...
    //       break
    //   }
    //   localStorage.setItem(tempStr, JSON.stringify(temp))
    // },
    // 要加在watch的内容
    ...
    top_list: {
      ...
      handler(newValue, old) {
        if (!old) {
          // this.linedata.series[0].name = newValue[0].unit
          // this.linedata.xAxis.name = '（' + this.top_list[0].unit + '）'
          this.linedata.series[0].name = this.top_list[0].unit
          this.kpi_name = newValue[0]['kpi_name']
          this.compute = newValue[0]['compute']
          // this.getTop5DataGroupByHost()
    ...
    activeName(newActiveName) {
      ...
      this.top_list.map((value, index) => {
        ...
        if (topN_title === newActiveName) {
          // this.linedata.xAxis.name = '（' + this.top_list[index].unit + '）'
          ...
          // this.linedata.series[0].name = unit
          // this.linedata.timeline.data = [1, 2]
          this.linedata.timeline.data = []
          // 判断页数
          for (let i = 0; i < Math.ceil(this.limitNum / 5); i++) {
            this.linedata.timeline.data.push(i + 1)
    ...
    resourceKeys(newActiveName, old) {
      ...
      // 长度没有发生改变调用方法
      this.getTop5DataGroupByHost() // 获取top5使用率主机以及值
  ...
  created() {
    ...
    // this.getTopNListToShow()//获取top列表
    // this.querySeverity() //查询告警级别
    // this.getTop5DataGroupByHost()
    // 设置单位默认值
    // this.linedata.series[0].name = this.top_list[0].unit
  },
  ...
  methods: {
    ...
    getAbnormalDevices() {
      ...
      div.scrollTop = 0
      this.msgInfo('已在设备列表为你检索出异常设备')
      this.operational_status = ['0', '2', '3']
      this.class_id_son = ''
      this.resourceName = ''
      this.findByresourceName()
    },
    addScroll(data) {
    ...
    getResourceSonClassId() {
      getResourceSonClassId({
        ...
        this.categoryListText = []
        this.categoryList.forEach((i) => {
          this.categoryListText.push(i.label_text)
        })
      })
    },
    getTop5DataGroupByHost() {
      // this.linedata.series[0].data = []
      // this.linedata.yAxis.data = []
      // 获取top5使用率主机以及值
      ...
      getTop5DataGroupByHost({
        ...
        if (!!temp && temp.length > 0) {
          temp.forEach((item, i) => {
            ...
            if (item.value) {
              if (this.linedata.options[page - 1]) {
                this.linedata.options[page - 1].series[0].data.unshift(parseInt(item.value))
                this.linedata.options[page - 1].yAxis[0].data.unshift(item.host)
              } else {
          ...
          // this.linedata.visualMap.max = this.linedata.options[0].series[0].data[this.linedata.options[0].series[0].data.length - 1]
          // this.linedata.visualMap.min = this.linedata.options[this.linedata.options.length - 1].series[0].data[0]
          // this.linedata.timeline.currentIndex = 0
          //  this.linedata.visualMap.min=this.linedata.options[this.linedata.options.length-1].series[0].data[this.linedata.options[0].series[0].data.length-1]
          // this.linedata.visualMap.max=this.linedata.options[this.linedata.options.length-1].series[0].data[this.linedata.options[this.linedata.options.length-1].series[0].data.length-1]
        }
      })
    },
    // 数值单位转换
    ...
    getNumByAlarmLevel() {
      ...
      getNumByAlarmLevel({
        resourceKeys: this.resourceKeys,
        projectId: 10,
        clabel_text: this.categoryListText,
        // nowDate: moment()
        //   .startOf('day')
        //   .format('x'),
        nowDate: this.nowDate1,
        ...
        if (!!nums && nums.length > 0) {
          // 1雷达图属性动态赋值
          // 1.1radarData radar indicator max : 取告警最多的为雷达图max
          // const maxTemp = nums.map((currentValue) => {
          //   return currentValue['dataValue']
          // })
          // let max = maxTemp.sort().reverse()[0]
          // let max=Math.max(...maxTemp)
          // 1.2radarData radar indicator
    ...
    turnConcern: function(row) {
      const userStr = localStorage.getItem('user')
      const userObj = JSON.parse(userStr)
      const userId = userObj['userId']
      const resourceId = row['resourceId']
      const isConcern = row.concern === 0 ? 1 : 0
      updatePropertyconcern({
        userId,
        resourceId,
        isConcern,
        projectId: 10,
    ...
    getResource() {
      ...
      getNetworkResource({
        ...
        cpuList: this.kpiDatasetName.cpuList, // cpu
        memList: this.kpiDatasetName.memList, // mem
        outTrafficList: this.kpiDatasetName.outTrafficList, // outTrafficList
        inTrafficList: this.kpiDatasetName.inTrafficList, // inTrafficList
        timeList: this.kpiDatasetName.timeList, // timeList
        classIdList: this.kpiDatasetName.classIdList, // classIdList
        isMainList: this.kpiDatasetName.isMainList,
        modelList: this.kpiDatasetName.modelList,
        cpuNumberList: this.kpiDatasetName.cpuNumberList,
        diskList: this.kpiDatasetName.diskList,
        versionList: this.kpiDatasetName.versionList, // 版本
        projectId: 10,
        engine: 'Freemarker'
      }).then((response) => {
        this.loading1 = false
